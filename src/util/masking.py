...
def get_indices_mask(indices_list, seq_len, mask_1d=True):
    mask = torch.zeros(seq_len).long()
    mask.scatter_(0,torch.tensor(indices_list).long(),\
        torch.ones(seq_len).long())
...
def get_missing_value_mask(value_mat, missing_value=-999):
...
def mask_from_indices_list(mask_list, len_full_list):
    mask = torch.zeros((len_full_list))
    mask.scatter_(0, torch.tensor(mask_list), torch.ones((len_full_list)))
    return mask.long()